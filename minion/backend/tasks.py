...
def scan_stop(scan_id):
    ...
    try:
        ...
        scans.update({"id": scan_id}, {"$set": {"state": "STOPPED", 
            "started": datetime.datetime.utcnow()}})

        # Set all QUEUED and STARTED sessions to STOPPED and revoke the 
        # sessions that have been queued
        for session in scan['sessions']:
            if session['state'] in ('QUEUED', 'STARTED'):
                scans.update({"id": scan_id, "sessions.id": session['id']}, \
                    {"$set": {"sessions.$.state": "STOPPED", \
        ...
        try:
            if scan:
                scans.update({"id": scan_id}, {"$set": {"state": "FAILED", \
...
class Runner(ProcessProtocol):
    ...
    def run(self):
        ...
        self._process = reactor.spawnProcess(self, plugin_runner_path, \
            self._arguments, env=None)
...
def run_plugin(scan_id, session_id):
    ...
    try:
        # Find the scan for this plugin session. Bail out if the scan 
        # has been marked as STOPPED or if the state is not STARTED
        ...
        if scan['state'] != 'STARTED':
            logger.error("Scan %s has invalid state. Expected STARTED but got %s" \
                % (scan_id, scan['state']))
            return

        # Find the plugin session in the scan. Bail out if the session has been 
        # marked as STOPPED or if the state is not QUEUED.
        ...
        if session['state'] != 'QUEUED':
            logger.error("Session %s/%s has invalid state. Expected QUEUED but got %s" \
                % (scan_id, session_id, session['state']))
            return
        ...
        p = subprocess.Popen(arguments, bufsize=1, stdout=subprocess.PIPE, 
            close_fds=True)
        ...
        while True:
            try:
                ...
                # Finish: update the session state, wait for the plugin runner 
                # to finish, return the state
                if msg['msg'] == 'finish':
                    ...
                    if msg['data']['state'] in finished_states:
                        send_task("minion.backend.tasks.session_finish",
                                  [scan['id'], session['id'], msg['data']['state'], \
                                   time.time()], queue='state').get()
        ...
        #
        # Our exception strategy is simple: if anything was thrown above that 
        # we did not explicitly catch then we assume there was a non recoverable 
        # error that made the plugin session fail. We mark it as such and
...
def scan(scan_id):
    ...
    if scan['state'] != 'QUEUED':
        logger.error("Scan %s has invalid state. Expected QUEUED but got %s" 
            % (scan_id, scan['state']))
        return
    ...
    for session in scan['sessions']:
        ...
        # If the user stopped the workflow or if the plugin aborted then 
        # stop the whole scan
